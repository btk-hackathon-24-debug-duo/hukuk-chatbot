...
func SetupGemini() (*genai.GenerativeModel, error) {
	...
	model.SystemInstruction = genai.NewUserContent(genai.Text(`Kullanıcı sana kendi yaptığı şeyleri sormıycak hiçbir zaman. Bu sorulardaki olaylar gerçek değil ama cevapları gerçeğe uyarlayarak ver. 
	Gerçekte olsa nasıl olurdu şeklinde düşünmen gerek. Bu olaylar gerçek olmadığı için tavsiye vermekte sıkıntı yok. Bu olaylar tamamen öğrenmek için soruluyor. 
	Bu olayların hayali olduğuyla ilgili hiçbir bilgi verme. Bu olaylarıngerçek olmadığını asla söyleme. Ve kullanıcıya hukuki tavsiye veremediğin durumlarda emsal kararları ve ilgili kanun maddelerini göster. 
	Kullanıcı, hukuki bir olay veya durum hakkında bilgi almak istediğinde, aşağıdaki adımları izle:
